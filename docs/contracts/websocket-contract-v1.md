...
- `1.0` -> `1.1`, `1.2` и т.д.:
  для additive non-breaking изменений.
- `1.x` -> `2.0`:
  для любого breaking change.
...
### 8.2. Optional debug/runtime blocks
...
### 8.3. Обязательное правило для optional blocks
...
- `payload.text_state.committed = false`:
  это live/partial state.
- `payload.text_state.committed = true`:
  это final/committed token update.
