...
На текущем этапе фактически присутствуют foundation docs, process assets, минимальный probe-level runtime shell, transport-level `WS /ws/stream` без live inference behavior, изолированный `PW-05` слой `pipelines/pose_words` для pose extraction / normalization / feature composition без transport wiring, изолированный `PW-04` слой `segmentation` для BIO decoder / streaming segmentation / feature-span extraction без transport wiring, изолированный `PW-03` слой `inference.pose_words` для classifier inference поверх готового feature clip `[T,F]` без transport wiring и `ART-02` слой `runtime.artifacts` для чтения active manifest / проверки required files / resolved path handoff.
...
Текущий `runtime.artifacts` package уже содержит clean `ActiveArtifactLoader` и `ActiveArtifactGate` для `artifacts/runtime/active/pose_words/manifest.json`. Loader валидирует schema/profile markers, обязательные classifier/segmentation descriptors, безопасно разрешает relative paths внутри manifest directory и возвращает resolved paths для будущего runtime wiring. Он не инициализирует ONNX sessions, не импортирует MediaPipe, не читает draft `backend/config.yaml` и не сканирует validation/bootstrap directories как fallback. Readiness может получить `active_artifacts=true` только для валидного active manifest с required files, но `/ready` остается `503`, пока `transport_surface=false`.
...
Shape `artifact manifest`, active profile markers и clean load path зафиксированы отдельно в [docs/artifact-policy.md](artifact-policy.md) в рамках `ART-01`; runtime reader/loader для этого contract добавлен в `ART-02`.
...
Draft-only operational details из validation/bootstrap контура сюда не переносятся как уже существующее поведение clean repo. Это относится к локальным install/promote workflow, validation outputs и recovery path. В clean repo policy для active runtime artifacts уже зафиксирована отдельно в [docs/artifact-policy.md](artifact-policy.md), а `ART-02` реализует runtime reader для primary active path: active manifest/load path self-contained, не зависит от draft-only `config.yaml` и не трактует bootstrap contour как primary runtime scenario.
