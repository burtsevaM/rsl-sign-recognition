...
Проверяемого десятого sample из согласованного списка пока нет: `работа`, `мама`, `папа` в этом source также не подтвердились как те же labels.
...
Если итог ниже `8/10`, это не нужно маскировать. Такой PR описывается как data investigation: bundle, manifest, runner и документация расширены, но сам active model setup еще не подтверждает достаточное покрытие demo dictionary.
...
Summary полного runner-а: `1/9 passed`.
...
4. Открыть отдельные data/model follow-up tasks, если нужен рост coverage или новый active pack.
